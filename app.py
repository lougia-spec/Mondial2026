...
# --- 1. LISTE DES 72 MATCHS ---
MATCHS = [
    {"id": 1, "groupe": "Groupe A", "eqA": "🇲🇽 Mexique", "eqB": "🇿🇦 Afrique Sud", "scA": 2, "scB": 2},
...
FICHIER_DB = "pronostics.csv"

# --- 2. FONCTIONS ---
def charger_donnees():
    if not os.path.exists(FICHIER_DB) or os.stat(FICHIER_DB).st_size == 0:
...
def calculer_points(prono_a, prono_b, reel_a, reel_b):
    ...
    # --- CORRECTION ICI : On force tout en nombres entiers (int) ---
    # Cela évite les bugs si un score est enregistré comme "1.0" ou "1" (texte)
    try:
        pa, pb = int(prono_a), int(prono_b)
        ra, rb = int(reel_a), int(reel_b)
    except:
        return 0 # Sécurité si erreur de données

    points = 0
    
    # 1. Qui a gagné ? (1 = A gagne, 2 = B gagne, 0 = Nul)
    res_reel = 1 if ra > rb else (2 if rb > ra else 0)
    res_prono = 1 if pa > pb else (2 if pb > pa else 0)
    
    # Si on a le bon vainqueur (ou le bon match nul)
    if res_reel == res_prono:
        points += 1 # 1 point de base
        
        # 2. Est-ce le score exact ?
        if pa == ra and pb == rb:
            points += 2 # +2 points bonus (Total 3)
...
with tab1:
    ...
    if valider:
        if not pseudo:
            st.error("⚠️ Il faut un pseudo pour jouer !")
        else:
            df = charger_donnees()
            df = df[df.Pseudo != pseudo] 
            nouveaux = []
            for mid, (sa, sb) in saisies.items():
                nouveaux.append({"Pseudo": pseudo, "Match_ID": mid, "Prono_A": sa, "Prono_B": sb})
            df = pd.concat([df, pd.DataFrame(nouveaux)], ignore_index=True)
            df.to_csv(FICHIER_DB, index=False)
            st.success(f"✅ C'est enregistré {pseudo} !")
            st.balloons()
...
with tab3:
    ...
    for grp in groupes:
        with st.expander(grp, expanded=False):
            # On extrait uniquement les équipes uniques
